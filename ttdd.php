...
<BR><BR>
<a href='ttdd.php?action=110&op=1'>Ler notas explicativas codclass.csv e carrega dados na tabela ttddnew1</a><BR><BR>
<a href='ttdd.php?action=110&op=13'>Ler dados na tabela ttddnew1 e gerar codclass.xml</a><BR><BR>
<a href='ttdd.php?action=106&op=7'>Arquivologia </a><BR><BR>
<a href='ttdd.php?action=109'>Converte onosmatico para formato xml do iphone</a><BR><BR>
<a href='ttdd.php?action=110'>Arquivologia nova Tabela</a><BR><BR>

<?php


	if (empty($_REQUEST["min"])) {$min=0;} else {$min=$_REQUEST["min"];}
	if (empty($_REQUEST["max"])) {$max=0;} else {$max=$_REQUEST["max"];}
	if (empty($_REQUEST["email"])) {$email='abrantes';} else {$email=$_REQUEST["email"];}
	if (empty($_REQUEST["data"])) {$data='2018-05-22';} else {$data=$_REQUEST["data"];}
	if (empty($_REQUEST["rpi"])) {$rpi=2472;} else {$rpi=$_REQUEST["rpi"];}
	if (empty($_REQUEST["gravar"])) {$gravar=0;} else {$gravar=$_REQUEST["gravar"];}
	if (empty($_REQUEST["ano"])) {$ano=date('Y');} else {$ano=$_REQUEST["ano"];}
	if (empty($_REQUEST["mes"])) {$mes=date('m');} else {$mes=$_REQUEST["mes"];}
	if (empty($_REQUEST["despacho"])) {$despacho="";} else {$despacho=$_REQUEST["despacho"];}
	if (empty($_REQUEST["revista"])) {$revista=2345;} else {$revista=$_REQUEST["revista"];}
	if (empty($_REQUEST["op"])) {$op=1;} else {$op=$_REQUEST["op"];}
	if (empty($_REQUEST["action"])) {$action=0;} else {$action=$_REQUEST["action"];}
	if (empty($_REQUEST["divisao"])) {$divisao='';} else {$divisao=$_REQUEST["divisao"];}
...
4) criar tabela ttddnew1 http://localhost/teste9.php?action=110&op=1
5) criar codclass.XML http://localhost/teste9.php?action=110&op=13

preparação do XML com o tabela de equivalÃªncia:
1) acertar o TXT
2) renomear para codequi.CSV
3) detectar algum ; faltante visualizando o CSV no Excel
4) criar tabela ttddnew2 http://localhost/teste9.php?action=110&op=2
5) cria html com tabela de equivalÃªncia: http://localhost/teste9.php?action=110&op=3
6) criar codequi.XML http://localhost/teste9.php?action=110&op=14

preparação do XML do esquema:
1) acertar o TXT
2) renomear para codsche.CSV
3) detectar algum ; faltante visualizando o CSV no Excel
4) criar tabela ttddnew3 http://localhost/teste9.php?action=110&op=4
5) testa se os campos de symbol da tabela ttddnew2 e ttdnew3 Ã© numÃ©rico: http://localhost/teste9.php?action=110&op=5
6) renomear symbol para grupo e criar campo symbol,CHAR 255
7) atualiza campo symbol com novo simbolo que reflete hierarquia: http://localhost/teste9.php?action=110&op=6
8) criar campos INT indice e proximo
9) http://localhost/teste9.php?action=110&op=7 para cada secao (ajustar manualmente com secao=0 veja qual o indice max e ajuste para depois secao=1)
10) temos que gerar dois xml: kind.xml com todos grupos, e kindS.xml somente som as subclasses http://localhost/teste9.php?action=110&op=8
11) gerar lista de simbolos validos de ttdnew3 e gravar no array simbolosValidos em Pesquisa.java http://localhost/teste9.php?action=110&op=9
...
preparação do XML do Ã­ndice de assuntos
1) acertar o onosmaequi.CSV
2) ler TXT http://localhost/teste9.php?action=110&op=10
3) conferir tabela ttddnew4 e acertar erros no TXT
5) gera indices http://localhost/teste9.php?action=110&op=11
6) gerar onosmaequi.XML http://localhost/teste9.php?action=110&op=12
7) ajuste onosmatico.java


*/

		if ($op==1) // http://localhost/teste9.php?action=110&op=1
		{
		...
		if ($op==2)
		{
			...
			// CREATE TABLE `producao`.`ttddnew2` (`id` INT NOT NULL ,`symbol1` CHAR( 10 ) NOT NULL ,`assunto1` TEXT NOT NULL ,`symbol2` CHAR( 10 ) NOT NULL ,`assunto2` TEXT NOT NULL  ) ENGINE = InnoDB
			$fname="codequi.csv";
			...
			else
			{
				...
				while (!feof($fpr))
				{
					...
					$assunto1 = trim($assunto1);
					$symbol2 = trim($symbol2);
					$assunto2 = trim($assunto2);
		...
		if ($op==4)
		{
			...
			// CREATE TABLE `producao`.`ttddnew3` (`id` INT NOT NULL ,`symbol` VARCHAR( 8 ) NOT NULL ,`kind` CHAR( 1 ) NOT NULL ,`assunto` TEXT NOT NULL ,`fase_corrente` CHAR( 255 ) NOT NULL ,`fase_intermediaria` CHAR( 255 ) NOT NULL ,`destinacao` CHAR( 255 ) NOT NULL ,`observacoes` TEXT NOT NULL ) ENGINE = InnoDB
			$fname="codsche.csv";
			...
			else
			{
				...
				while (!feof($fpr))
				{
					...
					$observacoes = trim($observacoes);
					$cmd = "INSERT INTO ttddnew3 (id,symbol,kind,assunto,fase_corrente,fase_intermediaria,destinacao,observacoes) VALUES ('$total','$symbol','$kind','$assunto','$fase_corrente','$fase_intermediaria','$destinacao','$observacoes');";
					$total++;
					$res = execute_query($cmd);
					echo "$cmd<BR>";
				}
		...
		if ($op==5)
		{
			...
			// ttddnew3 = esquema de navegação com todos os sÃ­mbolos
...
//				$res2=execute_query($cmd2);
//				if ($line2=@mysql_fetch_array($res2,MYSQL_ASSOC)) $explica = $line2['observacoes'];
...
*/
		...
		if ($op==10)
		{
			...
			// CREATE TABLE `producao`.`ttddnew4` (`id` INT NOT NULL ,`symbol` CHAR( 7 ) NOT NULL ,`grupo` CHAR( 7 ) NOT NULL, `kind` CHAR( 1 ) NOT NULL ,`assunto` TEXT NOT NULL,`subassunto` TEXT NOT NULL,`indice` INT(11) NOT NULL,proximo INT(11) ) ENGINE = InnoDB
			...
			else
			{
				...
				{
					$texto= trim(fgets($fpr));
					if (substr($texto,0,1)=='c') //c;1.;ABASTECIMENTO DE VEÃCULOS - ÃGUA
					{
		...
		if ($op==12)
		{
			...
			$texto = '</ROOT>';
			fputs($fpw,$texto."\n");
			echo "Fim de processamento: <a href='onosmafim.xml'>Clique aqui para onosmaequi.xml</a><BR><BR>";
			exit();
		}
...
/*****
    ...
    <item android:id="@+id/item_onosmatico"
        ...
        Para o Ã­ndice de assuntos foi utilizado o material produzido pela Comissão Permanente de Arquivo - CPArq da Universidade Federal da Bahia
...
2) renomear para CSV
3) detectar algum ; faltante visualizando o CSV no Excel
4) http://localhost/teste9.php?action=106&op=2
5) renomear symbol para grupo e criar campo symbol,CHAR 255
6) http://localhost/teste9.php?action=106&op=6
7) criar campos INT indice e proximo
8) http://localhost/teste9.php?action=106&op=1 para cada secao (ajustar manualmente)
9) http://localhost/teste9.php?action=106&op=3
10) gerar lista de sÃ­mbolos validos e gravar no array simbolosValidos em Pesquisa.java http://localhost/teste9.php?action=106&op=5
11) ajuste as classes iniciais em MainActivity.class e GruposPesquisa.class

preparação do XML ddo Ã­ndice de assuntos
1) acertar o TXT
2) ler no WORD e detectar erros ortograficos, acertar campos de ttddfim2
3) ler TXT http://localhost/teste9.php?action=106&op=8
4) conferir tabela ttddfim2 e acertar erros no TXT
5) gera indices http://localhost/teste9.php?action=106&op=10
6) gerar XML http://localhost/teste9.php?action=106&op=9
7) ajuste onosmatico.java
