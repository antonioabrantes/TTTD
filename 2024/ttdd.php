...
<?php
	...
	$user = "cienti42_root";
	$database  = "cienti42_producao";		// The name of database.
	$password = "037569ab";

	//$host = "localhost";		// Host name;
	//$user = 'root';
	//$database  = "producao";		// The name of database.
	//$password = "";
			...
			else
			{
				...
				while (!feof($fpr))
				{
					...
					else // 1.1;ABASTECIMENTO DE VEICULOS;042.4 ou 1.11;@;004
					{
						...
						$assunto = utf8_encode(trim($assunto)); // Acesso à documentação arquivistica Ver POLITICA DE ACESSO A DOCUMENTACAO ARQUIVISTICA
						$subassunto = '';
						$pos = strpos($assunto,' Ver ');
						if ($pos !== false)
						{
							$subassunto=substr($assunto,$pos+5); // POLITICA DE ACESSO A DOCUMENTACAO ARQUIVISTICA
							$assunto=substr($assunto,0,$pos); // Acesso à documentação arquivistica
						}
						$grupo = trim($grupo);
						$maingroup = $symbol;
						$symbol = $symbol.".";
...
preparaçao do XML do Indice de assuntos
1) acertar o TXT
		...
		if ($op==8)
		{
			...
			else
			{
				...
				while (!feof($fpr))
				{
					...
					else // 1.1;ABASTECIMENTO DE VEÍCULOS;042.4 ou 1.11;@;004
					{
						$kind = 0;
						list($symbol,$assunto,$grupo) = explode(';',$texto);
						$assunto = trim($assunto); // Acesso à documentação arquivística Ver POLÍTICA DE ACESSO À DOCUMENTAÇÃO ARQUIVÍSTICA
						$subassunto = '';
						$pos = strpos($assunto,' Ver ');
						if ($pos !== false)
						{
